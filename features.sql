...
create view vw_features_rows as 
...
create view vw_features as
select id,string_agg(feature,' ') as features from vw_features_rows 
group by id;
...
id_labels.votes,
id_labels.views,
'| n ' || replace(vw_features.features,':','---') as features
from vw_features
left join id_labels on vw_features.id = id_labels.id;


create table vw_all_train as 
select * from vw_all order by random();

-- create table vw_all_train as 
-- select * from vw_all order by random() limit 100000;

-- create table vw_all_test as 
-- (select * from vw_all) except (select * from vw_all_train);

create view vw_votes_train as 
select votes || ' ' || features from vw_all_train;
-- create view vw_votes_test as select id,votes,features from vw_all_test
-- order by id;

create view vw_comments_train as 
select comments || ' ' || features from vw_all_train;
-- create view vw_comments_test as select id,comments,features from vw_all_test
-- order by id;

create view vw_views_train as 
select views || ' ' || features from vw_all_train;
-- create view vw_views_test as select id,views,features from vw_all_test
-- order by id;

\! pwd

\copy (select * from vw_votes_train) to votes_train.vw
-- \copy (select features from vw_votes_test) to votes_test.vw
-- \copy (select votes from vw_votes_test) to votes.labels

\copy (select * from vw_comments_train) to comments_train.vw
-- \copy (select features from vw_comments_test) to comments_test.vw
-- \copy (select comments from vw_comments_test) to comments.labels

\copy (select * from vw_views_train) to views_train.vw
-- \copy (select features from vw_views_test) to views_test.vw
-- \copy (select views from vw_views_test) to views.labels

drop view vw_votes_train;
-- drop view vw_votes_test;
drop view vw_comments_train;
-- drop view vw_comments_test;
drop view vw_views_train;
-- drop view vw_views_test;
drop table vw_all_train;
-- drop table vw_all_test;
drop view vw_all;
drop view vw_features;
drop view vw_features_rows;
